...
class SOSuggestionEngine:
    """Generates Confluence page suggestions for Stack Overflow articles."""
    ...
    def __init__(self, confluence_docs: List[Document] = None):
        ...
        self.confluence_docs: List[Document] = confluence_docs or []

    def set_confluence_docs(self, confluence_docs: List[Document]) -> None:
        """
        Set or update the Confluence documents used for matching.
    ...
    def suggest_confluence_articles(
        self,
        so_document: Document,
        ...
        Returns:
            List of dicts: {title, url, page_id, relevance, reasoning}
        """
        if not self.confluence_docs:
            logger.warning("No Confluence documents loaded for suggestions")
            return []

        so_tags: List[str] = so_document.metadata.get("tags", [])
        so_title: str = so_document.metadata.get("title", "")
    ...
    def suggest_by_content_search(
        self,
        query: str,
        max_suggestions: int = 5,
    ) -> List[Dict]:
        """
        Suggest Confluence articles matching a free-text query.
        ...
        for doc in self.confluence_docs:
            ...
            matches = sum(
                title.count(word) * 3 + content.count(word)
...
def create_suggestion_engine(confluence_docs: List[Document] = None) -> SOSuggestionEngine:
    """
    Convenience function to create a suggestion engine.
...
if __name__ == "__main__":
    ...
    print(engine.format_suggestions(engine.suggest_confluence_articles(sample_so_doc)))
