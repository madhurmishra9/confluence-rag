...
SO_RATE_LIMIT_KEY = os.getenv("SO_RATE_LIMIT_KEY", "")  # Optional
...
def fetch_questions_by_tag(tag: str, min_answers: int = 1, limit: int = None) -> List[Dict]:
    ...
    try:
        while len(questions) < limit:
            ...
            # Respect rate limits (be polite to Stack Exchange API)
            import time
            time.sleep(0.5)
...
def questions_to_documents(questions: List[Dict], include_answers: bool = True) -> List[Document]:
    ...
    for question in questions:
        try:
            ...
            try:
                if created_date:
                    created_dt = datetime.fromtimestamp(created_date, tz=timezone.utc).isoformat()
                else:
                    created_dt = ""
            except:
                created_dt = ""
            ...
            # Optionally include top answers
...
def fetch_stackoverflow_questions(tags: str = None, include_answers: bool = True) -> List[Document]:
    """
    Fetch Stack Overflow questions and convert to documents.
    
    Args:
        tags: Comma-separated tags (default: STACKOVERFLOW_TAGS from .env)
        include_answers: Whether to include answers in documents
