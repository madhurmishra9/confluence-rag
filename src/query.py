...
Updated to use LCEL (LangChain Expression Language) instead of
the removed RetrievalQA chain (deprecated in langchain >= 0.2,
...
from langchain_ollama import ChatOllama                          # replaces langchain_community.llms.Ollama
...
# Custom prompt template — instructs LLM to answer ONLY from Confluence context
...
def get_llm() -> ChatOllama:
    ...
    Returns:
        ChatOllama configured with the llama3.1:8b model.
    """
    try:
        llm = ChatOllama(
...
def build_qa_chain(vectorstore) -> RunnableSerializable:
    """
    Build a RAG chain from the vector store using LCEL.

    Replaces the removed RetrievalQA.from_chain_type() pattern with an
    equivalent LCEL pipeline:
        retriever -> format_docs -> prompt -> llm -> str_parser

    The chain also stores retrieved source documents so callers can
    surface them alongside the answer (mirrors the old
    return_source_documents=True behaviour).

    Args:
        vectorstore: ChromaDB vector store instance.

    Returns:
        A RunnableSerializable chain that accepts {"question": str} and
    ...
    # LCEL chain — equivalent to RetrievalQA(chain_type="stuff", return_source_documents=True)
    # We split into two parallel branches so we can return both the answer and source docs.
    rag_chain_from_docs = (
        RunnablePassthrough.assign(context=(lambda x: _format_docs(x["source_documents"])))
        | prompt
        | llm
        | StrOutputParser()
    )

    # Full chain: retrieve docs, then answer + pass docs through
...
def ask(question: str, qa_chain: RunnableSerializable) -> Tuple[str, List[Document]]:
    """
    Ask a question using the QA chain and return the answer with sources.

    Args:
        question: The question to ask.
        qa_chain: The LCEL RAG chain built by build_qa_chain().
    ...
    try:
        # Invoke chain — input mirrors old {"query": question} but uses "question" key
        ...
        # Print sources (same behaviour as before)
